...
Install Nodejs Package:
  pkg.installed:
    - name: '{{ claude_code_tui.pkg.nodejs.name }}'
    - require:
      - pkg: Install Claude Code Agent OS Dependencies

Manage Global Node Modules Directory Permissions:
  file.directory:
    - dir_mode: '0755'
    - file_mode: '0755'
    - group: root
    - name: {{ claude_code_tui.path.node_modules }}
    - recurse:
      - group
      - mode
      - user
...
Install Claude Code Agent OS Dependencies:
  pkg.installed:
    - pkgs: {{ claude_code_tui.pkg.deps | json }}
